$('document').ready(function () {
    ...
    $('#masterData').on('click', '#master_edit', function (e) {
        ...
        window.location.href = "/masters/" + id + "/edit";
    });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (i = 0; i <= x.length; i++) {
                    var tr = $("<tr></tr>");
                    var td = $("<td><input class='' type='checkbox' name='students_id[]' value='" + x[i].id + "'></td><td>" + x[i].admission_no + "</td><td>" + x[i].first_name + "(" + x[i].last_name + ") </td><td>" + x[i].class.name + "(" + x[i].section.name + ") </td><td>" + x[i].parent + "</td><td>" + x[i].mobile + "</td>");
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success == true) {
                    ...
                    window.location.href = url+ "/examtypes";
                }
                else {
                    for (const key in data.errors) {
                        errorContainer = $('#' + key + '_error');
    ...
    function calculateTotal(arr, property) {
        ...
        for (let i = 0; i < arr.length; i++) {
            total += parseInt(arr[i][property]); // or parseFloat for decimal values
        }
        return total;
    }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (i = 0; i < x.length; i++) {
                        $("#progressData").css("display", 'block');
                        var tr = $("<tr></tr>");
                        var td = $("<td>" + x[i].first_name + " " + x[i].last_name + " </td><td>" + x[i].total_sum + "</td><td> " + x[i].marks_sum + "</td><td> " + [i + 1] + "</td>");
                        tr.append(td);
