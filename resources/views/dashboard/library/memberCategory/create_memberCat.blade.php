...
<div class="container py-5 px-5">
            ...
            <div class="page_hading_bredcrumb">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb_bg">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('member-category.index') }}">Create Member Category</a></li>
    ...
    <!-- Page Content -->
    <form action="#" method="POST" class="card" id="create_memberCat_form">
        <fieldset>
            @csrf
            <div class="inputs_data row card-body py-5">
                <div class="col-12 col-md-6">
                    <div class="mb-3">
                        <label for="memberCat" class="form-label label_name">Name <span class="text-danger fillable">*</span></label>

                        <input required type="text" class="form-control inputs" minlength="3" name="member_cat_name" value="{{ old('member_cat_name') }}" id="memberCat" placeholder="Enter name" required>
...
<script>
    $('#create_memberCat_form').validate();

    $(document).ready(function() {
        // Add Data to DB
        $('#create_memberCat_form').submit(function(e) {
            e.preventDefault();
  var isValid = true;
        ...
        if (isValid) {
            $.ajax({
                url: "{{ route('member-category.store') }}",
                type: "POST",
                data: $(this).serialize(),
                dataType: "json",
                success: function(response) {
                    if (response.success == true) {
                        $('#create_memberCat_form')[0].reset();
                        alert('Member category added Successfully');
                    }
                },
