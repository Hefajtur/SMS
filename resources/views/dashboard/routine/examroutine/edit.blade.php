...
@section('body')
    ...
    <div class="container py-5 ">
        <div class="row">
            <div class="col-md-11 mt-5 bg-white mx-5 py-3">
                <div class="">
                    <h3 class="bradecrumb-title mb-1 px-2 mt-2">Edit Exam Routine</h3>
                    <ol class="breadcrumb  bg-white">
                        <li class="breadcrumb-item" style="font-size: 15px"><a href="">Home</a></li>
                        <li class="breadcrumb-item" style="font-size: 15px">Edit Exam Routine</li>
                        <li class="breadcrumb-item" style="font-size: 15px">Edit</li>
                    </ol>
                </div>
                @foreach ($examRoutines as $examRoutine)
                @endforeach
                <form action="" method="POST" id="examRoutine_update" class_id="{{ $examRoutine->class->id }}" section_id="{{ $examRoutine->section->id }}">
                    <fieldset>
                        @csrf
                        ...
                        <div class="row mt-5">
                            <div class="col-md-3 mb-3">
                                ...
                                <select name="class_id" class="form-control inputs" id="class_id" required>
                                    ...
                                    <option value="{{ $class->id }}" {{ $examRoutine->class_id == $class->id ? 'selected' : ''}}>{{ $class->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select name="section_id" class="form-control inputs" id="section_id" required>
                                    <option value="{{ $examRoutine->section_id }}">{{ $examRoutine->section->name }}
                                    </option>
                                </select>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select name="type" class="form-control inputs" id="type" required>
                                    @foreach ($examTypes as $examType)
                                        <option value="{{ $examType->id }}" {{ $examRoutine->type == $examType->id ? 'selected' : ''}}>
                                            {{ $examType->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <input type="date" name="date" value="{{$examRoutine->date}}" class="form-control inputs" id="date" required>
    ...
    <script>
        $(document).ready(function() {
            var count = {{ count($examRoutines) }}; // Set the initial count to the number of existing rows.
            ...
            @foreach ($examRoutines as $key => $examRoutine)
                ...
                //pass hidden examRoutines id
                existingRow +=
                    "</select><input type='hidden' name='examRoutine_id[]' value='{{ $examRoutine->id }}'></td>";
                ...
                $('#subject_id{{ $key + 1 }}').val('{{ $examRoutine->subject->id }}');
                $('#time_schedule_id{{ $key + 1 }}').val('{{ $examRoutine->timeschedule->id }}');
                $('#class_room_id{{ $key + 1 }}').val('{{ $examRoutine->classroom->id }}');
            @endforeach
        });
    </script>
    ...
    <script>
        $('#examRoutine_update').validate();

        $(document).ready(function() {

            $('#examRoutine_update').submit(function(e) {
                e.preventDefault();
                ...
                if (isValid) {
                    $.ajax({
                        url: url + "/examRoutineUpdate/" + class_id + "/" + section_id,
                        method: "POST",
                        data: $(this).serialize(),
                        // data:{
                        //     'class_id': class_id,
                        ...
                        success: function(response) {
                            ...
                            if (response.success == true) {
                                ...
                                window.location.href = url + '/examRoutine';
                            }
                        },
