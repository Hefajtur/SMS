...
<div class="container py-5 px-5">
            ...
            <div class="page_hading_bredcrumb">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb_bg">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('online-exam-type.index') }}">Online Examination</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('question-group.index') }}">Question Group</a></li>
    ...
    <form action="#" method="POST" class="card" id="qstGroup_form">
        <fieldset>
            @csrf
...
<script>
    ...
    $(document).ready(function() {
        // Add Data to DB
        $('#qstGroup_form').submit(function(e) {
            e.preventDefault();
            ...
            if (isValid) {
                $.ajax({
                    url: "{{ route('question-group.store') }}",
                    type: "POST",
                    dataType: "json",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    data: $('#qstGroup_form').serialize(),
                    success: function(data) {
                        if (data.success == true) {
                            alert("Question Group added successfully");
                            $('#qstGroup_form')['0'].reset();
                        }
                    },
